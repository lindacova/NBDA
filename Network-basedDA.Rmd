...
```{r metadata_2, fig.show="hold", out.width="50%"}
ggplot(info_samples, aes(x=Group, fill=Sex))+
  geom_bar()+
  scale_fill_manual(values = my_colors[c(6,7)])

ggplot(info_samples, aes(x=Group, y=Age, fill=Group))+
  geom_boxplot()+
  scale_fill_manual(values = my_colors[c(3,4)])
```
...
The best mtry results to be 281.2747. Next, a model is built on the training set with these parameters and evaluated on the test set.
...
heatmap(imp25_data, scale="row", col=hmcol, ColSideColors=csc,
        ...
        labCol = FALSE)
# legend(x=0.7, y=1.07, legend=c("normal", "tumor"), fill=my_colors[c(3,5)])
legend(x=0.73, y=1, legend=c("normal", "tumor"), fill=my_colors[c(3,5)])
